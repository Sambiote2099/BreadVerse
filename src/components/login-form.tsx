...
export function LoginForm({
  ...
  // Handle URL parameters (verified status, messages)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        // Check if it's an OAuth-linked account error
        if (result.error.includes("linked to") || result.error.includes("registered with")) {
          setError(result.error);
        } else if (result.error.includes("verify your email")) {
          setError(result.error + " Check your inbox and spam folder.");
        } else {
  ...
  return (
    <div ref={sectionRef} className={cn("flex flex-col gap-4 md:mt-8 mt-14 scale-95", className)} {...props}>
            ...
            <FieldGroup>
              <div className="flex flex-col items-center gap-2 text-center">
                ...
                <p className="text-muted-foreground text-balance">
                  Login to your BreadVerse account
                </p>
              </div>
              ...
              <FieldDescription className="text-center">
                Don&apos;t have an account? <a className="text-amber-500" href="/signup">Sign up</a>
              </FieldDescription>
            </FieldGroup>
          </form>
          <div className="bg-muted relative hidden md:block">
            <img
              src="https://images.unsplash.com/photo-1634831318569-0df6cdeac7da?q=80&w=1374&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
              alt="Image"
      ...
      <FieldDescription className="px-6 text-center text-amber-100">
        By clicking continue, you agree to our <a href="#" className="text-amber-300 hover:underline">Terms of Service</a>{" "}
        and <a href="#" className="text-amber-300 hover:underline">Privacy Policy</a>.
      </FieldDescription>
    </div>
  )
}