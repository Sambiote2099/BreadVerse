...
export async function POST(request: Request) {
  try {
    ...
    const product = await db.collection("gift_boxes").findOne({ 
    ...
    await db.collection("gift_boxes").updateOne(
      { _id: new ObjectId(productId) },
      { $set: { 
...
export async function DELETE(
  ...
  try {
    ...
    const result = await db.collection("ratings2").deleteOne({
      productId: new ObjectId(productId),
      userId: new ObjectId(userId)
    });
    ...
    const remainingRatings = await db.collection("ratings2")
      .find({ productId: new ObjectId(productId) })
      .toArray();
