...
// Add Product type definition at the top
type Product = {
  _id: string;
  name: string;
  description: string;
...
export default function ProductDetailPage() {
  ...
  const [product, setProduct] = useState<Product | null>(null); // Add type
 ...
 useEffect(() => {
    const fetchUserRating = async () => {
      if (isLoggedIn && params.id) {
        try {
          const ratingRes = await fetch(`/api/ratings?productId=${params.id}`, {
            credentials: 'include'
          });
          if (ratingRes.ok) {
...
const handleFavorite = async () => {
  ...
  try {
    const response = await fetch('/api/favourites', {
      ...
      body: JSON.stringify({ 
        itemId: params.id, 
        itemType: 'product', // Different type
        action 
      })
    });
...
const checkIfFavorited = async () => {
  ...
  try {
    const res = await fetch(`/api/favourites/check?itemId=${params.id}&itemType=product`, {
      credentials: 'include'
    });
...
const handleRateProduct = async (rating: number) => {
  ...
  useEffect(() => {
    const fetchRandomProducts = async () => {
      try {
        const res = await fetch('/api/products?limit=10'); // Fetch more to have enough to filter
        if (res.ok) {
          const data = await res.json();
    ...
    <div className="min-h-screen mt-16 bg-[#f3ecd8] dark:bg-[#4A4036] transition-colors duration-1000">
      <div ref={ProductDetailsRef} className="container mx-auto px-4 py-8">
        ...
        <div className="text-sm text-gray-500 mb-6">
          ...
          <Link href="/products" className="hover:text-amber-600 ml-2">Products</Link> /
        ...
        <section className="flex flex-col lg:flex-row gap-6 mt-4">
          {product.process && product.process.length > 0 && (
            <div className="mb-8 flex-1">
              <h3 className="text-lg font-semibold mb-3">Preparation Process</h3>
              <div className="space-y-3">
                {product.process.map((step: string, index: number) => (
                  <div key={index} className="flex items-start gap-3">
                    <div className="flex-shrink-0 w-6 h-6 rounded-full bg-amber-500 text-white flex items-center justify-center text-sm font-semibold">
                      {index + 1}
                    </div>
          ...
          <div className="flex justify-center lg:justify-end lg:flex-1">
            <div className="relative rounded-4xl h-[260px] sm:h-[340px] lg:h-[420px] hover:scale-102 hover:-rotate-2 transition-transform duration-700 w-full max-w-sm lg:w-2/3 overflow-hidden shadow-2xl">
              <Image
